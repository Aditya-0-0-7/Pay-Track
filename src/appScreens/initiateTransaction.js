...
function InitiateTransaction({transactionList,User,toggleRefresh,navigation})
{
        ...
        <SafeAreaView style={styles.container}>
            <LinearGradient colors={['#eeeeee','#eeeeee']} style={{height:height, width:width}}>
                ...
                <View style={{position:'relative',top:110, left:width*0.1,height:0.63*height}}>
                    ...
                    renderItem={({item}) => {
                        if(item.status===0 && item.Verification_ID==User.verificationId)
                        return(<Transaction navigation={navigation} item={item} code={1}/>)
                        }}/>
                </View>
                </ImageBackground>
            </LinearGradient> 
...
export default connect(mapStateToProps)(InitiateTransaction);